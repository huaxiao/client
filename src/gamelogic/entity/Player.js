 ...
 var Player = (function () {
    ...
    p.onDead = function(killer) {
        ...
        if(killer!=null){
            ...
            //         var type = parseInt(effectType);
            //         if(type == BuffType.IncKillBall || type == BuffType.IncAllScore)
            //             this.m_score *= buff[effectType];
            //     }
            // }
            this.m_score *= killer.getBuffVal(BuffType.IncKillBall);
        }
        ...
        if(target.getEntityType() == EntityType.EnergyBall) {
            ...
            { 
                if(this.hittedPlayer(target)){
                    ...
                    //         var type = parseInt(effectType);
                    //         if(type == BuffType.IncKillScore || type == BuffType.IncAllScore) {
                    //             reward *= this.m_buffdata[effectType];
                    //         }
                    //     }
                    // }
                    reward *= this.getBuffVal(BuffType.IncKillScore);
    ...
    p.getBuffVal = function(type){
        return this.m_buffdata[type] || 1;
    }
    ...
    p.setGrowProp = function(grow){
        for(var id in grow) {
            ...
            var type = cfg.bufftype;
            var val = this.m_buffdata[type] || 1;
            val = val + cfg.buffval/100;
            this.m_buffdata[type] = val;
            if(type == BuffType.IncAllScore) {
                val = this.m_buffdata[BuffType.IncKillBall] || 1;
                val = val + cfg.buffval/100;
                this.m_buffdata[BuffType.IncKillBall] = val;

                val = this.m_buffdata[BuffType.IncKillScore] || 1;
                val = val + cfg.buffval/100;
                this.m_buffdata[BuffType.IncKillScore] = val;
            }
            else if(type == BuffType.IncMoveSpeed) {
                this.calcMoveSpeed();
            }
            else if(type == BuffType.IncAtkSpeed) {
                this.calcAtkSpeed();
            }
            else if(type == BuffType.IncAtkDistance) {
                this.setWeaponData();
                if(this.m_iActor)
