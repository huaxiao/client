...
var MiniServer = (function () {
    ...
    p.uploadBattleResult = function(msg){
        var battleType = msg.battleType,score=msg.score,task=msg.task;
        var curTime = Date.now();
        var user = GameData.getInstance().user;
        var goods = GameData.getInstance().goods;
        ...
        var gold = 0;
        var growId = 3;
        var growLv = user.grow[growId] || 0;
        var cfg = DataMgr.getInstance().getGrowPropCfg(growId,growLv);
        var addPercent = cfg&&cfg.buffval||0;
        if(battleType == GameMode.Grade) {
            var gradeData = DataMgr.getInstance().getGradeDataByStar(user.star);
            gold = this.calGoldByGradeStar(gradeData.grade,task,addPercent);
            if(task == 1) {
                user.setStar(user.star + 1);
            }
        }
        else {
            gold = this.calGoldByBattleScore(score,addPercent);
        }
    ...
    p.calGoldByBattleScore = function(score,addPercent){
        var gold = Math.floor(score * this.ScoreGoldRatio);
        gold = Math.ceil(gold * addPercent / 100) + gold;
    ...
    p.calGoldByGradeStar = function(grade,star,addPercent){
